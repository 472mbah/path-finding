...
from utility.util import addNode, inBucket as inBucketArea, sendRaw, convertIntoKeys
import time
# import rospy
...
def runSingleIteration (currentInfo, queue, reserves, visitedTrace, visited, vistedNext, end, obstacles, destArea, iterations):
...
def heuristic(start, end, barriers, aS, bS): #function for something else
    global obstacles
    obstacles = barriers
    ...
    while len(queue[0]) and len(queue[1]) and distanceApart(currents[0]['value'], currents[1]['value']) > 1:
        try:
            nextIndex = (index+1) % 2
            status = runSingleIteration (currents[index], queue[index], reserves[index], traces[index], visited[index], visited[nextIndex], currents[nextIndex]['value'], obstacles, destAreas[index], iterations[index])
            if status == -1:
                return (-1, -1)
            if type(status) == tuple:
                mergePoint = status
                break
            print('converted', convertIntoKeys(obstacles))
            ...
            sendRaw(stepDetails, (0,0,0,(25,25)), convertIntoKeys(obstacles), True)
            time.sleep(.01)
            iterations[index] += 1
            index = nextIndex
        except:
